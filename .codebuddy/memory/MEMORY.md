...
- **MonoBehaviour 序列化字段 NRE 根因（08-21 已实锤）+ image 复用方案已回退**：**根因**——Unity MonoManager 的 `m_ScriptImages` 启动时经 `il2cpp_domain_assembly_open` 按 image 指针注册一次、重载不刷新；HybridCLR 重载新建 image 使复用类 `klass->image` 变成未知新指针 → `GetAssemblyIndexFromImage` 返回 -1 → `CanTransferTypeAsNestedObject` false → 嵌套 managed 对象字段（如 `UIVariableArray.s_currVariable`）被序列化指令静默剔除 → prefab 反序列化跳过 → NRE（实锤探针 `ImageRegCheck`，il2cpp-api.cpp `il2cpp_class_get_flags`）。**image 指针复用修复已回退**：重载路径改 `ass=oldAss; image2=oldAss->image` 会让 `RestoreCachedGenericClassesPass1(old,new)` 两参同指针，而泛型还原（`ShouldRestoreGenericClass`/`RehashGenericClassSet` 的 `gclass->type` 归一化）依赖"新旧 image 不同指针"区分新旧来重映射陈旧元数据 → 归一化失效 → 泛型实例 `typeMetadataHandle` 越界崩溃（GetTypeRawIndex 是指针减法）。**要兼容须把泛型还原改造成按 InterpreterImage 索引（非 Il2CppImage 指针）区分新旧**——大改、风险高，未做。
