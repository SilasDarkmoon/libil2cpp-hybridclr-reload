	...
	const MethodInfo* GetMethodInfoFromMethodDef(const Il2CppType* type, const Il2CppMethodDefinition* methodDef)
	{
		...
		// ==={{ AssemblyReloadReuse: log pointer mismatch
		{
			...
			if (mname && strstr(mname, "GetLogger"))
			{
				...
				if (fp) {
					fprintf(fp, "[ReuseDiag] GetMethodInfoFromMethodDef FAIL: method='%s' methodDef=%p klass_ptr=%p klass='%s.%s' method_count=%u klass_image=%p typeMetaHandle=%p type_typeHandle=%p\n",
